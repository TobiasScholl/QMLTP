...
tff(subset_union, axiom, {$box}((! [B:$i]: ({$box}((! [C:$i]: ({$box}(({$box}(subset(B,C)) => {$box}(qmltpeq(union(B,C),C))))))))))).
...
tff(prove_idempotency_of_union, conjecture, {$box}((! [B:$i]: ({$box}(qmltpeq(union(B,B),B)))))).
